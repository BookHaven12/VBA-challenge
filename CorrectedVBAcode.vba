...
Sub Mult_year_stock_data()
    ...
    For Each ws In Worksheets
        ...
        ' Set headers for summary table & Greatest increase, decrease, and volume
        ...
        For i = 2 To lastrow
            ...
            If i = lastrow Or ws.Cells(i + 1, 1).Value <> ws.Cells(i, 1).Value Then
                ...
                Else
                    ws.Range("J" & Summary_Table_Row).Interior.ColorIndex = xlNone ' No color
                End If
                
                ' color formatting for Percent Change
                If PercentChange > 0 Then
                    ws.Range("K" & Summary_Table_Row).Interior.ColorIndex = 4 ' Green
                ElseIf PercentChange < 0 Then
                    ws.Range("K" & Summary_Table_Row).Interior.ColorIndex = 3 ' Red
                Else
                    ws.Range("K" & Summary_Table_Row).Interior.ColorIndex = xlNone ' No color
                End If
                ...
                ' Reset valuesfor FirstOpen, Qchange, and Total Stock Vol
                If i < lastrow Then
            ...
            Else
                ' Add to the total stock volume if the same ticker
                StockTotal = StockTotal + ws.Cells(i, 7).Value
            End If
    ...
    Next ws
    MsgBox "Quarterly Calculations Complete"
